...
export const getProductsForCollections = async (searchText, cursorDetails) => {
  const querySearchtext = searchText ? searchText : "";
  console.log("cursorDetails: ", cursorDetails);
  ...
  query MyQuery($querySearchtext: String, $cursor: String) {
    productsConnection(${beforeOrAfter === "before" ? "last" : "first" }: 3, ${beforeOrAfter}: $cursor, where: {_search: $querySearchtext}, orderBy: createdAt_DESC) {
        edges {
          cursor
          node {
            collections {
