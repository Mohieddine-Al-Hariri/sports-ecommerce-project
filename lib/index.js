...
export const getProducts = async (cursor, searchText, category, collection , isAdmin, onlyOnSale) => {
  ...
  const categoryQuery = category ? `, categories_some: {name: $category}` : "";
  const collectionQuery = collection ? `, collections_some: {name: $collection}` : "";
  const isOnSaleQuery = onlyOnSale ? `, isOnSale: true` : "";

  const query = gql`
  query MyQuery($querySearchtext: String, $cursor: String${category ? `, $category: String ` : ""}${collection ? `, $collection: String ` : ""}) {
    productsConnection(first: 9, after: $cursor, where: {_search: $querySearchtext${getPublished}${categoryQuery}${collectionQuery}${isOnSaleQuery}}, orderBy: createdAt_DESC) {
        edges {
          cursor
          node {
            collections {
...
export const getCollections = async (cursor, searchText, isAdmin, category) => {
  ...
  const hasAvailableProducts = isAdmin ? "" : "state: Available";
  const categoryQuery = (category && category !== "Collections & Sales") ? `categories_some: { name: "${category}" }` : "";
  const combined = (isAdmin || category) ? `, products_some: { ${hasAvailableProducts}, ${categoryQuery} }` : "";
...
// export const getCollections = async (cursor, searchText, isAdmin, category) => {//TODO: FINISH
//   const querySearchtext = searchText ? searchText : ""; 
//   const getPublished = !isAdmin ? ", state_not: Removed, products_some: {state: Available}" : "";
//   const categoryQuery = category ? `, products_some: {categories_some: {name: ${category}}}` : "";
//   let productSome = !isAdmin ? ", state_not: Removed, products_some: {state: Available}" : "";

//   console.log(category)
//   const query = gql`
//     query MyQuery($querySearchtext: String${category ? `, $category: String ` : ""}) {
